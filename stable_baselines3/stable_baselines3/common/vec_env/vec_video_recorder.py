...
class VecVideoRecorder(VecEnvWrapper):
    ...
    def __init__(
        ...
        self.video_name = None  # Initialize as None
    ...
    def step_wait(self) -> VecEnvStepReturn:
        ...
        if self.recording:
            ...
            if len(self.recorded_frames) > self.video_length:
                # Generate a dynamic name based on step_id
                self.video_name = f"{self.name_prefix}-step-{self.step_id - len(self.recorded_frames)}-to-step-{self.step_id}.mp4"
                self.video_path = os.path.join(self.video_folder, self.video_name)
    ...
    def _stop_recording(self) -> None:
        ...
        if len(self.recorded_frames) == 0:
            ...
            if self.use_wandb:
                wandb.log({"video": wandb.Video(self.video_path)},step=self.step_id)
