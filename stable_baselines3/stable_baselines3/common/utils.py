...
def safe_mean(arr: Union[np.ndarray, list, deque]) -> float:
    """
    Compute the mean of an array after removing NaN values if there is at least one non-NaN element.
    For arrays with no valid elements or empty arrays, return NaN. It is used for logging only.

    :param arr: Numpy array or list of values
    :return: Mean of the values excluding NaNs, or NaN if no valid values exist
    """
    if len(arr) == 0:
        return np.nan
    
    # Convert to numpy array to handle NaN filtering
    arr = np.array(arr)
    
    # Filter out NaN values
    valid_values = arr[~np.isnan(arr)]
    
    # Return NaN if no valid values exist after filtering
    return np.nan if len(valid_values) == 0 else float(np.mean(valid_values))
